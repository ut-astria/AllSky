...
void pass_prediction()
{
	...
	*/
	...
	std::vector <int> PassPhoto_RSOid;
	std::vector <double> PassPhoto_start_MJD;
	std::vector <double> PassPhoto_stop_MJD;
	std::vector <float> PassPhoto_topo_el_max_rad;
	std::vector <std::vector <aef> > PassPhoto_track_topo_rad;
	std::vector <std::vector <float> > PassPhoto_track_shadow;
	
	std::vector <int> PassFull_RSOid;
	std::vector <double> PassFull_start_MJD;
	std::vector <double> PassFull_stop_MJD;
	std::vector <float> PassFull_topo_el_max_rad;
	std::vector <std::vector <aef> > PassFull_track_topo_rad;
	std::vector <std::vector <float> > PassFull_track_shadow;
	...
	plot_pass_topo_radial(&PassFull_start_MJD, &PassFull_track_topo_rad, &PassFull_track_shadow);
			...
			if ((*Tracks_topo_rad)[iPass].size() > 0)
			{
				...
				for (int i = 0; i < (*Tracks_topo_rad)[iPass].size(); i++)
				{
					...
					if ((x >= 0) && (x < iW) && (y >= 0) && (y < iH))
					{
						...
						COLORREF PointCol = RGB(int(col.x), int(col.y), int(col.z));
