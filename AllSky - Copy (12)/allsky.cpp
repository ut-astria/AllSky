...
void plot_pass_topo_radial(
	std::vector <double> *Tracks_MJD,
	std::vector <std::vector <aef> > *Tracks_topo_rad,
	std::vector <std::vector <float> > *Tracks_shadow
);

void Polar_projection(
	HDC *hDC,
	RECT *ch,
	int ChartType,//0: CW_0Top (Az/El), 1: CCW_0Bottom (RA/Dec)
	int CoDec_deg,
	int Dec_step_deg,
	bool Lines_on_top,
	//data to plot
...
void Draw_Polar_chart(
	...
	double Dec_scale,
	int BMP_radius,
	COLORREF BackCol,//background color
	COLORREF TxtCol,//txt,points color
	COLORREF ScaleCol,//scale lines
	bool Extra_lines,//extended RA lines
	bool Clean_chart,
	bool Draw_Southern_hemisphere,
	int ChartTypeScale,//0: Az/El; 1: Lon/Lat; 2: RA/Dec; 3: Az/El count Az from-S-to-E SENW and don't mark SENW; 4: RA/Dec ad don't mark VE; 5: 0-360 clock style, ticks outside the chart
	bool EquatorInRed,
	bool ReverseDecScaleTxt//0=normal sign +-, 1=reversed signed -+
);

void Give_XY_for_RADec_or_AzEl(
	int ChartType,//0: CW_0Top (Az/El), 1: CCW_0Bottom (RA/Dec)
	double RA_deg,
	double Dec_deg,
	double Dec_max_deg,//Polar_Dec_max_deg
	double Dec_scale,//Polar_Dec_scale
...
void pass_prediction()
{
	...
	*/
			...
			else
			{
				...
				if ((temp_pass_start_MJD >= Night_start_MJD) && (temp_pass_stop_MJD >= temp_pass_start_MJD))
				{
					...
					if (photometric_pass_only)
					{
						...
						for (double MJD_pass = temp_pass_start_MJD; MJD_pass <= temp_pass_stop_MJD; MJD_pass += pass_fine_search_step_day)
						{
							...
							else
							{
								...
								{
									//add sunlit pass
									PassPhoto_RSOid.push_back(iRSO);
									PassPhoto_start_MJD.push_back(SunlitStart_MJD);
									PassPhoto_stop_MJD.push_back(SunlitStop_MJD);

									float el_max = 0.;									
									PassPhoto_track_topo_rad.push_back(std::vector<aef>());
									PassPhoto_track_shadow.push_back(std::vector<float>());
									int pass_number = PassPhoto_track_topo_rad.size() - 1;
									PassPhoto_track_topo_rad[pass_number].resize(0);
									PassPhoto_track_shadow[pass_number].resize(0);

									sat_pass_calculate_track(
										SunlitStart_MJD, SunlitStop_MJD,
										...
										search_step_s,
										&PassPhoto_track_topo_rad[pass_number],
										&PassPhoto_track_shadow[pass_number],
										&el_max
									);
									
									PassPhoto_topo_el_max_rad.push_back(el_max);
					...
					else
					{
						...
						{
							PassPhoto_RSOid.push_back(iRSO);
							PassPhoto_start_MJD.push_back(temp_pass_start_MJD);
							PassPhoto_stop_MJD.push_back(temp_pass_stop_MJD);
							...
							sat_pass_calculate_track(
								temp_pass_start_MJD, temp_pass_stop_MJD,
								Observer_ITRFm, Observer_LonLat_rad,
								EphSat_start_MJD, EphSat_step_s, EphSat_steps, &EphSat_ICS_m,
								EphSun_start_MJD, EphSun_step_s, EphSun_steps, &EphSun_ICS_m,
								search_step_s,
				...
				if (photometric_track_added)
				{
					...
					PassFull_RSOid.push_back(iRSO);
					PassFull_start_MJD.push_back(MJD_start);
					PassFull_stop_MJD.push_back(MJD_stop);

					float el_max = 0.;
					PassFull_track_topo_rad.push_back(std::vector<aef>());
					PassFull_track_shadow.push_back(std::vector<float>());
					int pass_number = PassFull_track_topo_rad.size() - 1;
					PassFull_track_topo_rad[pass_number].resize(0);
					PassFull_track_shadow[pass_number].resize(0);
					...
					sat_pass_calculate_track(
						...
						EphSun_start_MJD, EphSun_step_s, EphSun_steps, &EphSun_ICS_m,
						search_step_s,
						&PassFull_track_topo_rad[pass_number],
						&PassFull_track_shadow[pass_number],
						&el_max
					);

					PassFull_topo_el_max_rad.push_back(el_max);
	...
	//time sort data
	std::vector <int> PassFullSorted_RSOid;
	std::vector <double> PassFullSorted_start_MJD;
	std::vector <double> PassFullSorted_stop_MJD;
	std::vector <float> PassFullSorted_topo_el_max_rad;
	std::vector <std::vector <aef> > PassFullSorted_track_topo_rad;
	std::vector <std::vector <float> > PassFullSorted_track_shadow;

	PassFullSorted_RSOid.resize(0);
	PassFullSorted_start_MJD.resize(0);
	PassFullSorted_stop_MJD.resize(0);
	PassFullSorted_topo_el_max_rad.resize(0);
	PassFullSorted_track_topo_rad.resize(0);
	PassFullSorted_track_shadow.resize(0);

	if (PassFull_start_MJD.size() > 0)
	{
		for (int iPass = 0; iPass < PassFull_start_MJD.size(); iPass++)			
			if (PassFull_start_MJD[iPass]>30000.)
			{
				double MJD_min = PassFull_start_MJD[iPass];
				int iMin = -1;

				for (int jPass = iPass; jPass < PassFull_start_MJD.size(); jPass++)
					if ((PassFull_start_MJD[jPass] > 30000.)&&(PassFull_start_MJD[jPass] < MJD_min))
					{
						MJD_min = PassFull_start_MJD[jPass];
						iMin = jPass;
					}

				if (iMin >= 0)
				{
					PassFullSorted_RSOid.push_back(PassFull_RSOid[iMin]);
					PassFullSorted_start_MJD.push_back(PassFull_start_MJD[iMin]);
					PassFullSorted_stop_MJD.push_back(PassFull_stop_MJD[iMin]);
					PassFullSorted_topo_el_max_rad.push_back(PassFull_topo_el_max_rad[iMin]);

					PassFullSorted_track_topo_rad.push_back(std::vector<aef>());
					PassFullSorted_track_shadow.push_back(std::vector<float>());
					int jPass = PassFullSorted_RSOid.size() - 1;
					PassFullSorted_track_topo_rad[jPass].resize(0);
					PassFullSorted_track_shadow[jPass].resize(0);
					for (int i = 0; i < PassFull_track_topo_rad[iMin].size(); i++)
					{
						PassFullSorted_track_topo_rad[jPass].push_back(PassFull_track_topo_rad[iMin][i]);
						PassFullSorted_track_shadow[jPass].push_back(PassFull_track_shadow[iMin][i]);
					}
				}
	...
	PassFull_RSOid.resize(0);
	PassFull_start_MJD.resize(0);
	PassFull_stop_MJD.resize(0);
	PassFull_topo_el_max_rad.resize(0);
	PassFull_track_topo_rad.resize(0);
	PassFull_track_shadow.resize(0);




	//plot_pass_topo_radial(&PassPhoto_start_MJD, &PassPhoto_track_topo_rad, &PassPhoto_track_shadow);
	plot_pass_topo_radial(&PassFull_start_MJD, &PassFull_track_topo_rad, &PassFullSorted_track_shadow);
	
	//list passes
	printf("\n");
	if (PassPhoto_start_MJD.size()>0)
	{ 
		for (int iPass = 0; iPass < PassPhoto_start_MJD.size(); iPass++)
		{
			double pass_duration_s = (PassPhoto_stop_MJD[iPass] - PassPhoto_start_MJD[iPass])*86400.;
			int iRSO = PassPhoto_RSOid[iPass];

			printf("Pass %d, %s, ", iPass+1, TLEcat_Name[iRSO].c_str());
			print_date_time(PassPhoto_start_MJD[iPass]);
			printf(" - ");
			print_date_time(PassPhoto_stop_MJD[iPass]);
			printf(", duration %.0lf s, max el %4.1lf\370\n", pass_duration_s, PassPhoto_topo_el_max_rad[iPass]*rad2deg);
			//degree sign: \370
		}
	}
	...
	PassPhoto_RSOid.resize(0);
	PassPhoto_start_MJD.resize(0);
	PassPhoto_stop_MJD.resize(0);
	PassPhoto_topo_el_max_rad.resize(0);
	PassPhoto_track_topo_rad.resize(0);
	PassPhoto_track_shadow.resize(0);
...
)
{
	...
	RECT rect_topo;//polar
	rect_topo = rect;

	int ChartType = 0;//0: CW_0Top (Az/El), 1: CCW_0Bottom (RA/Dec)
	int CoDec_deg = 90;
	int Dec_step_deg = 10;
	bool Lines_on_top = true;
	
	if ((*Tracks_MJD).size() > 0)
		if ((*Tracks_MJD).size() == (*Tracks_topo_rad).size())
			Polar_projection(
				&hDC,
				&rect_topo,
				ChartType,//0: CW_0Top (Az/El), 1: CCW_0Bottom (RA/Dec)
				CoDec_deg,
				Dec_step_deg,
				Lines_on_top,
				//data to plot
...
{
	int Chart_left = (*ch).left;//x of top-left corner
	int Chart_top = (*ch).top;//y of top-left corner
	int Chart_side = (*ch).bottom - (*ch).top;
	int Chart_radius = Chart_side / 2;

	int TxtHeight = 18;
	int Dec_min_deg = 90 - CoDec_deg;
	int Dec_max_deg = 90;
	double Dec_scale = Calculate_picture_scale(Dec_min_deg, Dec_max_deg, double(Chart_radius));

	int RA_step_deg = 30;
	int ChartTypeScale = 5;
	//0: Az/El; 1: Lon/Lat; 2: RA/Dec; 3: Az/El count Az from-S-to-E SENW and don't mark SENW; 4: RA/Dec ad don't mark VE
	//5: 0-360 clock style, ticks outside the chart

	bool Draw_text = false;
	bool extended_RA_lines = 0;
	...
	Draw_Polar_chart(
		...
		extended_RA_lines,
		0,//clean bmp
		0,//draw southern hemisphere
		ChartTypeScale,//ChartTypeScale: 0: Az/El; 1: Lon/Lat; 2: RA/Dec; 3: Az/El count Az from-S-to-E SENW and don't mark SENW; 4: RA/Dec ad don't mark VE
		0, 0);
	...
	//plot tracks
	if (((*Tracks_topo_rad).size() > 0) && ((*Tracks_topo_rad).size() == (*Tracks_shadow).size()))
	{		
		for (int iPass = 0; iPass < (*Tracks_topo_rad).size(); iPass++)
			if ((*Tracks_topo_rad)[iPass].size() > 0)
			{
				int ix_prev = -1;
				int iy_prev = -1;
				bool first = true;

				for (int i = 0; i < (*Tracks_topo_rad)[iPass].size(); i++)
				{
					//x y 
					float az = (*Tracks_topo_rad)[iPass][i].az;
					float el = (*Tracks_topo_rad)[iPass][i].el;
					int ix = 0, iy = 0;

					Give_XY_for_RADec_or_AzEl(
						ChartType,
						az*rad2deg,
						el*rad2deg,
						Dec_max_deg,
					...
					{
						double factor = (*Tracks_shadow)[iPass][i];
	...
	int end_Dec_step_deg = 90;
	int end_RA_step_deg = 0;
	if (Lines_on_top)
	{
		end_Dec_step_deg = Dec_step_deg;
		end_RA_step_deg = RA_step_deg;
	}
	...
	Draw_Polar_chart(
		...
		ChartTypeScale,//ChartTypeScale: 0: Az/El; 1: Lon/Lat; 2: RA/Dec; 3: Az/El count Az from-S-to-E SENW and don't mark SENW; 4: RA/Dec ad don't mark VE
		0,//bool EquatorInRed
		0);//bool ReverseDecScaleTxt//0=normal sign +-, 1=reversed signed -+	

	/*		   
	//plot extra points
	if ((*Point_rad).size() > 0)
		for (int i = 0; i < (*Point_rad).size(); i++)
		{
			int x = 0, y = 0;
			Give_XY_for_RADec_or_AzEl(
				ChartType,
				(*Point_rad)[i].az*rad2deg,
				(*Point_rad)[i].el*rad2deg,
				Dec_max_deg,
				Dec_scale,
				Chart_radius,
				2 * Chart_radius,
			...
			{
				COLORREF PointCol = clBlack;
				if (i == 0) PointCol = clBlack; else
					if (i == 1) PointCol = clGray;
...
{
	if (Clean_chart)
		FillRect(*hDC, DrawRect, CreateSolidBrush(BackCol));

	//XY axis
	//SelectObject(*hDC, CreatePen(PS_SOLID, 1, clBlack));	 
	// Radial_BMP->Canvas->MoveTo(DrawRect.Left+BMP_radial_chart_radius,DrawRect.Top);
	// Radial_BMP->Canvas->LineTo(DrawRect.Left+BMP_radial_chart_radius,DrawRect.Bottom);
	// Radial_BMP->Canvas->MoveTo(DrawRect.Left,DrawRect.Top+BMP_radial_chart_radius);
	// Radial_BMP->Canvas->LineTo(DrawRect.Right,DrawRect.Top+BMP_radial_chart_radius);

	int Xpix, Ypix;
	for (int Dec_deg = Dec_min_deg; Dec_deg <= Dec_max_deg; Dec_deg += Dec_step_deg)
		for (double RA_deg = 0.; RA_deg < 360.; RA_deg += 0.1)
		{
			Give_XY_for_RADec_or_AzEl(1, RA_deg, Dec_deg, Dec_max_deg, Dec_scale, BMP_radius, 2 * BMP_radius, &Xpix, &Ypix);

			/*
			if (Dec_deg==0)
			SetPixel(*hDC, (*DrawRect).left+Xpix, (*DrawRect).top+Ypix, clWhite);
			else
			*/
			SetPixel(*hDC, (*DrawRect).left + Xpix, (*DrawRect).top + Ypix, ScaleCol);

			/*
			//RA arrow
			if ((Dec_deg==Dec_min_deg)&&(RA_deg<45.))
			{
			 SetPixel(*hDC, Xpix, Ypix-1, TxtCol);
			 SetPixel(*hDC, Xpix, Ypix, TxtCol);
			}
			*/
		}

	//RA radii
	double Temp_dec_min_deg = Dec_min_deg;
	if (Extra_lines)
		Temp_dec_min_deg -= 10.;

	if (RA_step_deg > 0)
		for (double Dec_deg = Temp_dec_min_deg; Dec_deg <= Dec_max_deg; Dec_deg += 0.01)
			for (int RA_deg = 0; RA_deg < 360; RA_deg += RA_step_deg)
			{
				Give_XY_for_RADec_or_AzEl(1, RA_deg, Dec_deg, Dec_max_deg, Dec_scale, BMP_radius, 2 * BMP_radius, &Xpix, &Ypix);
				SetPixel(*hDC, (*DrawRect).left + Xpix, (*DrawRect).top + Ypix, ScaleCol);
				...
				if (RA_deg==0.)
				{
					SetPixel(*hDC, Xpix-1, Ypix, TxtCol);
					SetPixel(*hDC, Xpix, Ypix, TxtCol);
				}
				*/
			}
	...
	if (Draw_text)
	{
		...
		//scale names
		//skala polar: Dec
		int Dec_min = Dec_min_deg;
		int Dec_max = Dec_max_deg;

		if (ChartTypeScale == 2)//mark VE, chart type RA/Dec
		{
			//mark direction on VE
			double Dec_VEline_start_deg = Dec_min;
			double Dec_VEline_end_deg = Dec_min + (Dec_max - Dec_min) / 4.;
			int VEx0, VEy0, VEx1, VEy1;
			Give_XY_for_RADec_or_AzEl(1, 0., Dec_VEline_start_deg, Dec_max_deg, Dec_scale, BMP_radius, 2 * BMP_radius, &VEx0, &VEy0);
			Give_XY_for_RADec_or_AzEl(1, 0., Dec_VEline_end_deg, Dec_max_deg, Dec_scale, BMP_radius, 2 * BMP_radius, &VEx1, &VEy1);
			VEy0 -= 5;
			VEy1 -= 5;
			//arrow on the end of string
			int ArrowR = int(float(TxtHeight) / 4.);//5
			int ArrowH = int(float(TxtHeight) / 1.5);//18;

			int X0 = (*DrawRect).left + VEx0;
			int Y0 = (*DrawRect).top + VEy0;

			SelectObject(*hDC, CreatePen(PS_SOLID, 1, TxtCol));
			MoveToEx(*hDC, (*DrawRect).left + VEx0, (*DrawRect).top + VEy0, NULL);
			LineTo(*hDC, (*DrawRect).left + VEx1, (*DrawRect).top + VEy1);

			for (int dx = 0; dx <= ArrowR; dx++)
			{
				MoveToEx(*hDC, X0 - 1, Y0, NULL);
				LineTo(*hDC, X0 - 1 - dx, Y0 - ArrowH);

				MoveToEx(*hDC, X0, Y0, NULL);
				LineTo(*hDC, X0 + dx, Y0 - ArrowH);
			}

			MoveToEx(*hDC, X0 - 1 - ArrowR, Y0 - ArrowH, NULL);
			LineTo(*hDC, X0 + ArrowR, Y0 - ArrowH);

			//napisanie oznaczenia VE
			X0 = (*DrawRect).left + VEx0 - 50 - TxtHeight;
			Y0 = (*DrawRect).top + VEy0 - 40 - TxtHeight * 1.5;

			TextOut(*hDC, X0, Y0, TEXT("VE"), strlen("VE"));
		}

		if (ChartTypeScale == 5)
			Dec_min = Dec_max;

		for (int Dec_val = Dec_min; Dec_val <= Dec_max; Dec_val += Dec_step_deg)
		{
			sprintf_s(txt, "%s", "");

			if (ReverseDecScaleTxt)
				sprintf_s(txt, "%d°", -Dec_val);//char(0x0B0);//hex=0x0B0, dec=176, degree symbol
			else
				sprintf_s(txt, "%d°", Dec_val);//char(0x0B0);//hex=0x0B0, dec=176, degree symbol				

			if ((Draw_Southern_hemisphere) && (Dec_min == 0) && (Dec_max == 90) && (Dec_val != 0))
			{
				if (ReverseDecScaleTxt)
					sprintf_s(txt, "%d°", Dec_val);//char(0x0B0);//hex=0x0B0, dec=176, degree symbol
				else
			...
			int iRadialChartType = -1;

			if ((ChartTypeScale == 0) || (ChartTypeScale == 5))
			{
				//Az El: North=0Az, East=90Az, South=180Az, West=270Az, wykres Azimuth CW
				iRadialChartType = 0;//0: CW_0Top (Az/El)
			}
			else
			{
				//other chart types: RA - CCW
				iRadialChartType = 1;//1: CCW_0Bottom (RA/Dec)
			}

			Give_XY_for_RADec_or_AzEl(iRadialChartType, 0., double(Dec_val), Dec_max_deg, Dec_scale, BMP_radius, 2 * BMP_radius, &Xpix, &Ypix);

			Txt_pos_X = Xpix - 5;//+3;
			//Txt_pos_Y=Ypix-TxtHeight/2;
			Txt_pos_Y = Ypix - TxtHeight + 3;

			if ((Dec_val == 0) && (ChartTypeScale == 0))
				sprintf_s(txt, "%s", "");
			...
			int X0 = (*DrawRect).left + Txt_pos_X + 10;
			int Y0 = (*DrawRect).top + Txt_pos_Y;

			std::wstring stemp = s2ws(txt);
			//TextOut(*hDC, X0, Y0, stemp.c_str(), stemp.length());
			TextOut(*hDC, X0, Y0, txt, stemp.length());
		}

		if (EquatorInRed)
			if ((0 >= Dec_min_deg) || (0 <= Dec_max_deg))
			{
				//red circle to mark equator
				for (double RA_deg = 0.; RA_deg < 360.; RA_deg += 0.1)
				{
					Give_XY_for_RADec_or_AzEl(1, RA_deg, 0, Dec_max_deg, Dec_scale, BMP_radius, 2 * BMP_radius, &Xpix, &Ypix);
					SetPixel(*hDC, (*DrawRect).left + Xpix, (*DrawRect).top + Ypix, clRed);
				}
			}
		...
		if (ChartTypeScale == 5)
		{
			...
			for (int RA_val = RA_min; RA_val <= RA_max; RA_val += RA_step)
			{
				float az = float(RA_val)*deg2rad;
				sprintf_s(txt, "%d°", RA_val);//char(0x0B0);//hex=0x0B0, dec=176, degree symbol
				GetTextExtentPoint32(*hDC, (LPTSTR)txt, strlen(txt), &size);
				int tw = size.cx;
				int th = size.cy;
				int Dec_deg = -3;
				Give_XY_for_RADec_or_AzEl(0, RA_val, Dec_deg, Dec_max_deg, Dec_scale, BMP_radius, 2 * BMP_radius, &Xpix, &Ypix);
				Xpix = Round_value(float(Xpix) + float(tw)*(sin(az) - 1.) / 2.);
				Ypix = Round_value(float(Ypix) - float(th)*(1. + cos(az)) / 2.);
				std::wstring stemp = s2ws(txt);
				//TextOut(*hDC, (*DrawRect).left + Xpix, (*DrawRect).top + Ypix, stemp.c_str(), stemp.length());
				TextOut(*hDC, (*DrawRect).left + Xpix, (*DrawRect).top + Ypix, txt, stemp.length());
			}
		}
		else
			for (int RA_val = RA_min; RA_val <= RA_max; RA_val += RA_step)
			{
				if (ChartTypeScale == 0)//chart type: Az/El
				{
					sprintf_s(txt, "%d°", RA_val);//char(0x0B0);//hex=0x0B0, dec=176, degree symbol
					if ((RA_val == 0) || (RA_val == 360)) sprintf_s(txt, "%s", "N");
					if (RA_val == 90) sprintf_s(txt, "%s", "E");
					if (RA_val == 180) sprintf_s(txt, "%s", "S");
					if (RA_val == 270) sprintf_s(txt, "%s", "W");
				}
				else
					if ((ChartTypeScale == 1) && (RA_val > 0))//chart type: Lon/Lat
					{
						sprintf_s(txt, "%d°", RA_val);//char(0x0B0);//hex=0x0B0, dec=176, degree symbol			
					}
					else
						if ((ChartTypeScale == 2) && (RA_val > 0))//chart type: RA/Dec
						{
							sprintf_s(txt, "%dh", RA_val / 15);//h
						}
						else
							if ((ChartTypeScale == 3) && (RA_val > 0))//chart type: Az/El count Az from-S-to-E SENW and don't mark SENW
							{
								sprintf_s(txt, "%d°", RA_val);//char(0x0B0);//hex=0x0B0, dec=176, degree symbol			
							}
							else
								if ((ChartTypeScale == 4) && (RA_val > 0))//chart type: RA/Dec ad don't mark VE
								{
									sprintf_s(txt, "%dh", RA_val / 15);//h
								}
				...
				float radial_offset = 0.;
				if (ChartTypeScale == 5) radial_offset = 25.;
				double Radius_LAT_pix = (Dec_max_deg - Dec_min_deg + radial_offset)*Dec_scale;

				GetTextExtentPoint32(*hDC, (LPTSTR)txt, strlen(txt), &size);
				double Txt_width_pix = float(size.cx);
				double Txt_height_pix = float(size.cy);
				//txt diagonal
				double Diagonal_pix = 0.;
				double sqrt_argument = Txt_width_pix * Txt_width_pix + Txt_height_pix * Txt_height_pix;
				if (sqrt_argument >= 0.)
					Diagonal_pix = sqrt(sqrt_argument);

				//left top corner position
				double Txt_box_radius = Diagonal_pix / 2.;
				double Radius_reduced = Radius_LAT_pix - Txt_box_radius;
				//box center is on radius of Radius_reduced
				//XY coordinates
				double RA_val_rad = (RA_val - 90.)*deg2rad;
				double X_value = Radius_reduced * cos(RA_val_rad);
				double Y_value = Radius_reduced * sin(RA_val_rad);

				int Xpix, Ypix;
				if ((ChartTypeScale == 0) || (ChartTypeScale == 5))//Az El: North=0Az, East=90Az, South=180Az, West=270Az, wykres Azimuth CW
				{
				...
				else
				{
					//other chart types: RA - CCW
					Xpix = int(X_value) + BMP_radius;
					Ypix = (2 * BMP_radius - 1) - (int(Y_value) + BMP_radius);
				}

				//reduce position to left top corner
				Xpix -= int(Txt_width_pix / 2.);
				Ypix -= int(Txt_height_pix / 2.);

				int X0 = (*DrawRect).left + Xpix;
				int Y0 = (*DrawRect).top + Ypix;
				std::wstring stemp = s2ws(txt);
				//TextOut(*hDC, X0, Y0, stemp.c_str(), stemp.length());
				TextOut(*hDC, X0, Y0, txt, stemp.length());
			}
...
{
	double RA_rad = (RA_deg - 90.)*deg2rad;
	double Radius_LAT_deg = Dec_max_deg - Dec_deg;
	double Radius_LAT_pix = Radius_LAT_deg * Dec_scale;
	double X_value = Radius_LAT_pix * cos(RA_rad);
	double Y_value = Radius_LAT_pix * sin(RA_rad);
	*Xpix = int(X_value) + BMP_radius;
	if (ChartType == 0) *Ypix = int(Y_value) + BMP_radius;
	else
		if (ChartType == 1) *Ypix = (BMP_height - 1) - (int(Y_value) + BMP_radius);
}
