...
)
{
	...
	v3 Out_Sun_ICS_m;
	double shadow_function = shadow_function_Hubaux(//shadow function
		MJD,
		Sat_ICSm,
		Sun_ICS_rad,
...
void pass_prediction()
{
	...
	*/
				...
				if ((temp_pass_start_MJD >= Night_start_MJD) && (temp_pass_stop_MJD >= temp_pass_start_MJD))
				{
					...
					if (photometric_pass_only)
					{
						...
						for (double MJD_pass = temp_pass_start_MJD; MJD_pass <= temp_pass_stop_MJD; MJD_pass += pass_fine_search_step_day)
						{
							...
							double ShadowFunction = shadow_function_Hubaux(MJD_pass, Sat_ICS, Sun_ICS_rad);
							...
							{
								//in shadow
								if ((SunlitStart_MJD >= temp_pass_start_MJD) && (SunlitStop_MJD >= SunlitStart_MJD))
								{
									//add sunlit pass
									Pass_start_MJD.push_back(SunlitStart_MJD);
									Pass_stop_MJD.push_back(SunlitStop_MJD);
								}
								SunlitStart_MJD = -1.;
								SunlitStop_MJD = -1.;
							}
	...
	std::vector <float> Pass_topo_el_max_rad;
	Pass_topo_el_max_rad.resize(0);
	double step_day = 1. / 86400.;
	for (int iPass = 0; iPass < Pass_start_MJD.size(); iPass++)
	{			
		float el_max = 0.;
		for (double MJD = Pass_start_MJD[iPass]; MJD <= Pass_stop_MJD[iPass]; MJD += step_day)
		{
		...
		Pass_topo_el_max_rad.push_back(el_max);
	}

	//calculate tracks	
	double Tracks_step_s = 1.;
	double Tracks_step_day = Tracks_step_s / 86400.;
	std::vector <double> Tracks_MJD;
	std::vector <std::vector <aef> > Tracks_topo_rad;
	Tracks_MJD.resize(0);
	Tracks_topo_rad.resize(0);
	Tracks_topo_rad.resize(Pass_start_MJD.size());
	for (int iPass = 0; iPass < Pass_start_MJD.size(); iPass++)
	{
		Tracks_MJD.push_back(Pass_start_MJD[iPass]);
		Tracks_topo_rad[iPass].resize(0);
		for (double MJD = Pass_start_MJD[iPass]; MJD <= Pass_stop_MJD[iPass]; MJD += Tracks_step_day)
		{
			v3 ICS = eph_point(MJD, EphSat_start_MJD, EphSat_step_s, EphSat_steps, &EphSat_ICS_m);
			v3 TCS = ICS_to_TCS(MJD, ICS);//Inertial to Terrestrial
			ae topo = topo_AzEl_rad(Observer_ITRFm, Observer_LonLat_rad, TCS);
			...
			topof.az = topo.az;
			topof.el = topo.el;
			Tracks_topo_rad[iPass].push_back(topof);
		}
	}

	plot_pass_topo_radial(&Tracks_MJD, &Tracks_topo_rad);
	...
	{ 
		for (int iPass = 0; iPass < Pass_start_MJD.size(); iPass++)
		{
			...
			printf("Pass %d, start ", iPass+1);
			print_date_time(Pass_start_MJD[iPass]);
			printf(", stop ");
			print_date_time(Pass_stop_MJD[iPass]);
			printf(", duration %6.1lf s, max el %4.1lf\370\n", pass_duration_s, Pass_topo_el_max_rad[iPass]*rad2deg);
			//degree sign: \370
		}
	}
